defmodule Plug.Cowboy.SupervisorTest do
  ...
  test "supervisor drains connections correctly" do
    ...
    spec = {Plug.Cowboy, scheme: :http, plug: __MODULE__, options: [port: 8005, drain_timeout: 1000, drain_check_interval: 10]}
  ...
  defp observe_slow_request() do
    this = __MODULE__
    Task.async(fn ->
      {:ok, status, _headers, client} = :hackney.request(:get, "http://127.0.0.1:8005/", [], "", [:stream])
